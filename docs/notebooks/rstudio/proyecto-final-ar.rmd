...
- Número de renglones (**NOTE-08**): 
- Número de columnas (**NOTE-09**): 
- Densidad de la matriz (**NOTE-10**): 
- Media aritmética del número de elementos (itemsets) por transacción (**NOTE-11**):
...
- support (mínimo) (**NOTE-12**): 
- confidence (mínimo) (**NOTE-13**):
- items (máximo) (**NOTE-14**):
- tiempo de verificación de subsets (máximo) (**NOTE-15**):
...
- Cantidad de reglas producidas (**NOTE-16**):
- Media aritmética de support (**NOTE-17**):
- Media aritmética de confidence (**NOTE-18**): 
...
- Calcula el valor de la métrica support para la regla `{beer} => {nuts}` (**NOTE-20**): 
- Calcula el valor de la métrica confidence para la regla `{beer} => {nuts}` (**NOTE-21**): 
- ¿Por qué no aparece listada la regla `{beer} => {nuts}`? (**NOTE-22**): 
...
- ¿Cuál es el propósito de `session id`? (**NOTE-23**): 
- ¿Cuál es el propósito de `track id`? (**NOTE-24**):
- ¿Consideras que las otras columnas del dataset son aplicables para análisis por medio de reglas de asociación, justifica tu respuesta? (**NOTE-25**): 
...
- ¿Cuántos items tiene el itemset con mayor número de ocurrencias en la lista element length distribution? (**NOTE-27**): 
- En el contexto de este dataset con información de sesiones de streaming, ¿cuál es la interpretación de la media aritmética? (**NOTE-28**): 
...
- ¿Qué factores están asociados con que el algoritmo `apriori` con parámetros por omisión no haya producido ninguna regla de asociación, justifica tu respuesta? (**NOTE-29**):
- ¿Qué riesgos identificas con la asignación de valores muy bajos en los parámetros de  `support` y `confidence` al invocar el algoritmo `apriori`?
