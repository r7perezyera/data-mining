...
Examina el tipo de datos de Groceries
```{r}
str(Groceries)
```

Observa que la clase de Groceries es 'transactions' y contiene tres slots identificados como @data, @itemInfo y @itemsetInfo

itemsetInfo es un data.frame con un renglón por transacción (cada transacción se considera un itemset). El data.frame puede contener columnas con información adicional, p.e. IDs de transacciones o de usuarios. Es acceder a sus elementos por medio de `transactionInfo` 

El método `itemInfo()` permite visualizar información de items específicos de un objeto transactions.
```{r}
itemInfo(Groceries[1:5])
```
...
```{r}
STUDENT_ID = 0 #Sustituir por los dígitos de tu matrícula (p.e. 430000).
```
